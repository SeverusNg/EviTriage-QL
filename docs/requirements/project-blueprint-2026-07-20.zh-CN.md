...
> **中英文档维护要求**：所有人工维护的项目文档必须同时提供英文和简体中文版本，并在同一次变更中保持语义一致。英文文档沿用 `*.md`，中文对应文件使用 `*.zh-CN.md`；以中文为原文的规范性文档应提供相邻的 `*.en.md`。每对文档顶部必须提供语言切换链接，标题、链接、命令、示例、警告、版本号、能力边界和已知限制不得出现实质性偏差。README、贡献指南、安全策略、已知限制、CHANGELOG、架构与运行指南、ADR、进度与发布说明、fixture 说明以及本蓝图/构建要求均在范围内；自动生成文件、第三方原文、许可证、引用元数据和不可变运行产物除外。
