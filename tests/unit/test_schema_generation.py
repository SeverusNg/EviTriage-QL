...
def test_schema_generation_is_deterministic(tmp_path: Path) -> None:
    ...
    assert {path.name for path in (tmp_path / "schemas").iterdir()} == {
        ...
        "existing-sarif-experiment-manifest.schema.json",
        "experiment-preflight.schema.json",
        ...
        "resource-analyst-output.schema.json",
        "resource-judge-output.schema.json",
        "resource-rebuttal-output.schema.json",
