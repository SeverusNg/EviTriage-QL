...
套件、资源泄露 V2 离线实现及历史发布边界。只有经过明确授权的远程运行、
不可变聚合和延后基线评估都存在后，才可声称 RocketMQ V2 DeepSeek 实验完成。
以下条目是有意的范围边界或未解决验证缺口，不是完成声明。

## 资源泄露 V2 边界

- 资源提取是受限词法分析，不是 Java 编译器、AST/CFG、别名分析或跨过程
  所有权证明。关键字候选必须经过模型 claim 与确定性策略，不能直接升级为
  已验证语义。
- 同文件一跳 callee 受数量和 token 预算限制。缺失、重载、动态分派、生成、
  仅字节码或第三方行为会成为显式 context gap，通常强制 NMC。
- 尚未实现 `javap`/JAR 语义适配器；系统不会在线下载第三方源码，也不允许
  用模型常识替代可追溯证据。
- 全路径释放覆盖、获取成功、可行退出、资源身份、所有权和生命周期契约取决于
  现有受限证据。unknown/conflicting 评估或部分源码会无视模型置信度强制 NMC。
- 实验运行器仅支持顺序 existing-SARIF，没有 continuation/checkpoint；RocketMQ
  缺少必需的已检入 Maven Wrapper，因此实验有意不使用 `--scan`。
- 延后 V1 比较衡量工程 agreement，不是无偏准确率：基线是人工证据复核，
  不是独立验证的绝对 ground truth，而且开发者已经看过 V1 案例。

此前 Gate G 边界还包括确定性的六案例离线矩阵、可单独选择的攻击类别
